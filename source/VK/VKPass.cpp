...
void PathTracePass::CreatePipeline(Device& device)
{
	...
	m_MissShader = device.CreateShader("assets/system/shaders/Vulkan/bin/pathtrace.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR);
	m_RayGenShader = device.CreateShader("assets/system/shaders/Vulkan/bin/pathtrace.rgen.spv", VK_SHADER_STAGE_RAYGEN_BIT_KHR);
	m_AnyHitShader = device.CreateShader("assets/system/shaders/Vulkan/bin/pathtrace.rahit.spv", VK_SHADER_STAGE_ANY_HIT_BIT_KHR);
	m_MissShadowShader = device.CreateShader("assets/system/shaders/Vulkan/bin/shadow.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR);
	m_ClosestHitShader = device.CreateShader("assets/system/shaders/Vulkan/bin/pathtrace.rchit.spv", VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR);
...
void PathTracePass::Record(const Device& device, const Viewport& viewport, VkCommandBuffer commandBuffer, const BindlessDescriptorSet& bindlessTextures)
{
	...
	EXT::vkCmdTraceRaysKHR(commandBuffer, &raygen_region, &miss_region, &hit_region, &callable_region, viewport.GetRenderSize().x, viewport.GetRenderSize().y, 1);
}
