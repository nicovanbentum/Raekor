...
Raekor::Camera::Camera(glm::vec3 position, float fov) :
...
bool Raekor::Camera::is_mouse_active() {
    return mouse_active;
}
    
void Raekor::Camera::set_mouse_active(bool state) {
    mouse_active = state;
}
    
glm::vec3 Raekor::Camera::get_direction() {
    return glm::vec3(cos(angle_v) * sin(angle_h), 
    sin(angle_v), cos(angle_v) * cos(angle_h));
}
    
void Raekor::Camera::add_look(int x, int y) {
    angle_h += look_speed * (x * -1);
    angle_v += look_speed * (y * -1);
}

void Raekor::Camera::move_on_input(float amount) {
    auto dir = get_direction();
