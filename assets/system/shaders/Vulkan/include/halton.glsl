...
uint haltonIndex(uint x, uint y, uint i) {
    return ((halton2Inverse(x % 256, 8) * 76545 +
             halton3Inverse(y % 256, 6) * 110080) % 2 /* m_increment? */) + i * 186624;
}

void haltonInit(inout HaltonState hstate, int x, int y, int frameId) {
    hstate.dimension = 2;
    hstate.sequenceIndex = haltonIndex(x, y, frameId);
}
